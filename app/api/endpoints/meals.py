...
async def create_meal(meal: MealCreate, request: Request, db: Annotated[Client, Depends(get_auth_db)]):
    try:
        response = db.auth.get_user()
        user = response.user
        meal_data = MealService.create_meal(db, meal)
        await RedisService.expire(f"meals:{user.id}", expire=0)
        return meal_data
...
async def update_meal(meal_id: int, meal: MealUpdate, request: Request, db: Annotated[Client, Depends(get_auth_db)]):
    try:
        response = db.auth.get_user()
        user = response.user
        ...
        await RedisService.expire(f"meal:{user.id}:{meal_id}", expire=0)
        await RedisService.expire(f"meals:{user.id}", expire=0)
...
async def delete_meal(meal_id: int, request: Request, db: Annotated[Client, Depends(get_auth_db)]):
    response = db.auth.get_user()
    user = response.user
